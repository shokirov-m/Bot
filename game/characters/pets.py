"""
Питомцы: бесплатный ритуал в храме «Тихий Ручей» (рынок, якорь 0), промокоды. Платного призыва в городе нет.
...
def format_city_hub_pets_hint_html(*, locale: str) -> str:
    ...
    _ = locale
    return (
        "🐾 <b>Питомцы:</b> первого даёт <b>храм призыва</b> в «Тихом Ручье» (рынок, один бесплатный ритуал). "
        "Пассив в бою — у каждого свой, активен <b>один</b>; смена — в «Статус» или на этажах 8 и 48."
    )
...
def set_active_pet(character: Character, key: str) -> tuple[bool, str]:
    ...
    if key not in owned:
        return False, "Сначала получи питомца (храм в «Тихом Ручье», промо и т.п.)."
    mp, st = _pets_meta(character)
    st["active"] = key
    _save_meta(character, mp, st)

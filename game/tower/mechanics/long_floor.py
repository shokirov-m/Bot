...
def spawns_for_tower_progress(character: Character, floor_number: int) -> list[FloorMonsterSpawn]:
    """Слоты для учёта «все цели этажа» (делегат → ``registry.spawns_for_tower_progress``)."""
    from game.tower.mechanics import registry

    return registry.spawns_for_tower_progress(character, floor_number)

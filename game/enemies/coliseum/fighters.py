...
class ColiseumFighter:
    ...
    accuracy: float | None = None  # доля 0..1, переопределение из JSON; None — формула по слоту
...
def coliseum_monster_accuracy_evasion_defaults(fighter: ColiseumFighter) -> tuple[float, float]:
    """Базовая точность / уклонение бойца арены (доли 0..1) по слоту 1–50."""
    fid = max(1, min(50, int(fighter.id)))
    t = (fid - 1) / 49.0
    acc = 0.015 + t * (float(MONSTER_ACCURACY_CAP) - 0.015)
    ev = 0.01 + t * (float(MONSTER_EVASION_CAP) - 0.01)
    acc = min(float(MONSTER_ACCURACY_CAP), max(0.0, acc))
    ev = min(float(MONSTER_EVASION_CAP), max(0.0, ev))
    if fighter.special == "monster_evasion_30":
        ev = max(ev, 0.30)
    return acc, ev
...
def build_coliseum_monster_bundle(fighter: ColiseumFighter) -> dict[str, Any]:
    ...
    da, de = coliseum_monster_accuracy_evasion_defaults(fighter)
    if fighter.accuracy is not None:
        da = min(float(MONSTER_ACCURACY_CAP), max(0.0, float(fighter.accuracy)))
    if fighter.evasion is not None:
        de = min(float(MONSTER_EVASION_CAP), max(0.0, float(fighter.evasion)))
    return {
        "name": fighter.name,
        "emoji": "⚔️",

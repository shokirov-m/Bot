...
def format_stat_derived_effects_ru(eff: dict[str, int], *, class_key: str | None = None) -> str:
    """
    ...
    from game.tower.progression.rewards import luck_drop_bonus
    ...
    dex_line = (
        ...
        "В бою у монстров есть <b>точность</b> (снижает твоё уклонение) и <b>уклонение</b> (повышает твой промах)."
    )
    if _f.is_assassin_path_class(class_key):
        dex_line += (
            " <i>Убийца/ассасин: ЛОВ выше капа уклонения дополнительно ослабляет точность и уклонение врага.</i>"
        )
...
def format_stat_cheat_sheet_ru() -> str:
    ...
    return (
        ...
        "🏃 <b>Ловкость</b> — уклонение и шанс попадать по врагу; в бою точность врага снижает уклонение, уклонение врага — попадание.\n"

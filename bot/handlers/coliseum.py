...
def _menu_html(char) -> str:
    ...
    return (
        "🏛️ <b>Колизей</b>\n"
        f"📊 Побед: <b>{prog}</b>/50 · "
        f"{'следующий <b>#' + str(nxt) + '</b>' if nxt else 'все повержены 🏆'}\n"
        "⚔️ Бои по порядку — нужен уровень и победа над предыдущим.\n"
        "👑 Каждый 10-й боец: ×2 золота и опыта.\n"
        f"<i>В списке сейчас: <b>#{a}–#{b}</b>.</i>"
    )

"""Некромант: покупка класса и ковчег скелетов."""
...
def _ritual_intro_html(char) -> str:
    ...
    return (
        ...
        f"• Базовый путь уже выбран\n\n"
        f"⚔️ <b>Навыки:</b>\n{sk_lines or '—'}\n\n"
        f"☠️ <b>Особенность:</b> до <b>{MAX_SKELETONS_IN_BATTLE}</b> скелетов в бою и колизее "
        f"вместо наёмников. Смена класса необратима.\n\n"
        f"{status}"
    ).replace(",", " ")
...
def _coffin_html(char) -> str:
    ...
    return (
        "⚰️ <b>Ковчег костей</b>\n\n"
        f"Выберите до <b>{MAX_SKELETONS_IN_BATTLE}</b> союзников (колосс занимает 2 слота). "
        "Нажмите, чтобы добавить или убрать из отряда:\n\n"
        + ("\n".join(lines) if lines else "• Нет открытых типов нежити.")
    )


@router.callback_query(F.data == "prf:necro")
async def on_necro_ritual_menu(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    try:
        ...
        if is_necromancer(char):
            await callback.answer("Вы уже некромант. Откройте ковчег костей.", show_alert=True)
            return
...
async def on_necro_buy(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    try:
        ...
        await push_game_ui(
            ...
            text=(
                f"🎉 {html.escape(msg)}\n\n"
                "Откройте <b>Ковчег костей</b> и выберите скелетов перед боем. "
                "Наёмники больше не сражаются с вами."
            ),

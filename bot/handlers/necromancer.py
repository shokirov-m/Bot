...
def _ritual_intro_html(char) -> str:
    ...
    return (
        ...
        f"⚔️ <b>Навыки:</b>\n{sk_lines or '—'}\n\n"
        f"☠️ <b>Особенность:</b> до <b>{MAX_SKELETONS_IN_BATTLE}</b> скелетов в бою "
        f"вместо наёмников. Наёмники в отряд больше не берутся.\n\n"
        f"{status}"
    ).replace(",", " ")
...
def _coffin_html(char) -> str:
    ...
    return (
        "⚰️ <b>Ковчег костей</b>\n\n"
        "Выберите до двух союзников (колосс занимает оба слота — позже). "

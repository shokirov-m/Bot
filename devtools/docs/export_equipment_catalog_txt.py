...
Выход: docs/catalog_equipment/*.txt
"""
...
from game.economy import shop as shop_module
from game.economy.shop import SHOP_GOODS, SHOP_PORTRAITS

SHOP_FLOOR3_GEAR = getattr(shop_module, "SHOP_FLOOR3_GEAR", ())
from game.tower.progression.floor_data import SECRET_ROOM_CHANCE
...
OUT_DIR = Path(__file__).resolve().parents[2] / "docs" / "catalog_equipment"
...
def _write_loot_normal() -> str:
    parts = [
        ...
        "• Срабатывает roll_item_drop из game/tower/progression/rewards.py.",
...
def _write_secret() -> str:
    ...
    parts = [
        "🔍 УСЛОВИЕ",
        "• На экране этажа есть действие «обыск / тайник» (services/progression/floor_service.try_secret_search).",
        ...
        f"• Если тайник найден — предмет из каталога в сумку с шансом ~{gear_if_secret_pct:.0f}% (если сумка не полна).",
        "",
        f"РЕЖИМ ЭТАЖЕЙ 1–{SECRET_GEAR_MAX_FLOOR}: процедурный предмет из каталога (game/items/equipment/secret_gear.py → catalog_loot).",
        "Фиксированный пул SECRET_GEAR_ITEMS удалён; см. content/data/items/ и catalog_generated.",
        "",
        f"Шанс по сценарию: ~{secret_room_pct:.0f}% × ~{gear_if_secret_pct:.0f}% на предмет при найденном тайнике.",
    ]
    return "\n".join(parts)
...
def _write_runes_doc() -> str:
    return "\n".join(
        [
            ...
            "После победы: отдельный бросок roll_rune_stone(spawn) в game/tower/progression/rewards.py",
...
def main() -> None:
    ...
    _write_text("19_РУНЫ_И_РУННЫЕ_КАМНИ.txt", "Руны и рунные камни", "game/tower/progression/rewards.py + game/items/runes.py", _write_runes_doc())
